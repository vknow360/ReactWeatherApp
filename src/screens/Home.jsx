    ...
    return (
        <div className="flex flex-col min-h-full w-full p-3 bg-[#162438] relative">
            ...
            <button 
                ...
                className="fixed bottom-6 right-6 bg-[#4361ee] text-white p-4 rounded-full shadow-lg hover:bg-[#3a56d4] transition-all duration-300 z-50"
            >
                {showChatbot ? <FaTimes size={24} /> : <FaRobot size={24} />}
            </button>

            {showChatbot && (
                <div className="fixed bottom-20 right-6 w-80 h-96 bg-[#1a1a2e] rounded-lg shadow-xl border border-[#2b2d42] flex flex-col z-50 overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                        {conversation.length === 0 ? (
                            <div className="text-center text-gray-400 h-full flex flex-col items-center justify-center">
                                <FaRobot size={48} className="mb-4 text-[#4361ee]" />
                                <p>Ask me anything about weather!</p>
                                <p className="text-sm mt-2">Try: "What should I wear in {location.name}?"</p>
                            </div>
                        ) : (
                            conversation.map((message, index) => (
                                <div 
                                    key={index} 
                                    className={`p-3 rounded-lg max-w-[80%] ${message.sender === 'user' ? 'bg-[#4361ee] text-white ml-auto' : 'bg-[#2b2d42] text-white mr-auto'}`}
                                >
                                    <div className="text-xs text-gray-300 mb-1">
                                        {message.sender === 'user' ? 'You' : 'AI'} • {new Date(message.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                    </div>
                    ...
                    <form onSubmit={handleAiSubmit} className="p-3 border-t border-[#2b2d42]">
                        <div className="flex">
                            <input
                                type="text"
                                value={aiInput}
                        ...
                        {aiError && (
                            <div className="text-red-400 text-xs mt-1">{aiError}</div>
                        )}
                        <button 
                            type="button"
                            onClick={clearAiChat}
